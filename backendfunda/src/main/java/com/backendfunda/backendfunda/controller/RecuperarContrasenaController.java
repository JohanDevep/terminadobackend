    ...
    public ResponseEntity<String> resetearContrasena(@RequestParam("token") String token, @RequestParam("password") String nuevaContrasena) {
        ...
        try {
            ...
            if (usuarioOptional.isPresent()) {
                ...
                if (!nuevaContrasena.matches(".*[0-9].*") || !nuevaContrasena.matches(".*[A-Z].*")) {
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("La nueva contraseña debe contener al menos un número y una mayúscula.");
                }
